...
public interface ${ENTITY_NAME}Service {
    ...
    Optional<${ENTITY_NAME}> findById(<#if ENTITY_PRIMARY_KEYS?? && (ENTITY_PRIMARY_KEYS?size gt 1)>${ENTITY_NAME}.${ENTITY_NAME}Id<#else>String</#if> id);
...
<#if !(ENTITY_PRIMARY_KEYS?? && (ENTITY_PRIMARY_KEYS?size gt 1))>
    ...
    void delete(<#if ENTITY_PRIMARY_KEYS?? && (ENTITY_PRIMARY_KEYS?size gt 1)>${ENTITY_NAME}.${ENTITY_NAME}Id<#else>String</#if> id);

    void delete(List<<#if ENTITY_PRIMARY_KEYS?? && (ENTITY_PRIMARY_KEYS?size gt 1)>${ENTITY_NAME}.${ENTITY_NAME}Id<#else>String</#if>> id);
